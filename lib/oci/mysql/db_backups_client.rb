...
module OCI
  ...
  class Mysql::DbBackupsClient
    ...
    #
    # @param [OCI::Mysql::Models::CopyBackupDetails] copy_backup_details Details required to copy a given DB system backup to another region.
    # @param [Hash] opts the optional parameters
    # @option opts [OCI::Retry::RetryConfig] :retry_config The retry configuration to apply to this operation. If no key is provided then the service-level
    #   retry configuration defined by {#retry_config} will be used. If an explicit `nil` value is provided then the operation will not retry
    # @option opts [String] :if_match For optimistic concurrency control. In the PUT or DELETE call for a
    ...
    # @note Click [here](https://docs.cloud.oracle.com/en-us/iaas/tools/ruby-sdk-examples/latest/mysql/copy_backup.rb.html) to see an example of how to use copy_backup API.
    def copy_backup(copy_backup_details, opts = {})
      logger.debug 'Calling operation DbBackupsClient#copy_backup.' if logger

      raise "Missing the required parameter 'copy_backup_details' when calling copy_backup." if copy_backup_details.nil?
      ...
      post_body = @api_client.object_to_http_body(copy_backup_details)

      # rubocop:disable Metrics/BlockLength
      OCI::Retry.make_retrying_call(applicable_retry_config(opts), call_name: 'DbBackupsClient#copy_backup') do
        @api_client.call_api(
          :POST,
          path,
          endpoint,
