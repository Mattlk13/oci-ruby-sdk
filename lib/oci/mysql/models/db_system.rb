...
module OCI
  ...
  class Mysql::Models::DbSystem
    ...
    # @return [OCI::Mysql::Models::RestDetails]
    attr_accessor :rest
    ...
    # **[Required]** The database mode indicating the types of statements that are allowed to run in the the DB system.
    ...
    # **[Required]** The access mode indicating if the database access is unrestricted (to all MySQL user accounts),
    ...
    # @return [OCI::Mysql::Models::ReadEndpointDetails]
    ...
    # @option attributes [OCI::Mysql::Models::RestDetails] :rest The value to assign to the {#rest} property
    ...
    # @option attributes [OCI::Mysql::Models::ReadEndpointDetails] :read_endpoint The value to assign to the {#read_endpoint} property
    ...
    def database_mode=(database_mode)
      # rubocop:disable Style/ConditionalAssignment
      if database_mode && !DATABASE_MODE_ENUM.include?(database_mode)
        OCI.logger.debug("Unknown value for 'database_mode' [" + database_mode + "]. Mapping to 'DATABASE_MODE_UNKNOWN_ENUM_VALUE'") if OCI.logger
        @database_mode = DATABASE_MODE_UNKNOWN_ENUM_VALUE
      else
    ...
    def access_mode=(access_mode)
      # rubocop:disable Style/ConditionalAssignment
      if access_mode && !ACCESS_MODE_ENUM.include?(access_mode)
        OCI.logger.debug("Unknown value for 'access_mode' [" + access_mode + "]. Mapping to 'ACCESS_MODE_UNKNOWN_ENUM_VALUE'") if OCI.logger
        @access_mode = ACCESS_MODE_UNKNOWN_ENUM_VALUE
      else
    ...
    def hash
      [id, display_name, description, compartment_id, subnet_id, nsg_ids, rest, is_highly_available, current_placement, is_heat_wave_cluster_attached, heat_wave_cluster, availability_domain, fault_domain, shape_name, mysql_version, backup_policy, source, configuration_id, data_storage_size_in_gbs, data_storage, hostname_label, ip_address, port, port_x, endpoints, channels, lifecycle_state, lifecycle_details, maintenance, deletion_policy, time_created, time_updated, freeform_tags, defined_tags, system_tags, crash_recovery, point_in_time_recovery_details, database_management, secure_connections, database_mode, access_mode, customer_contacts, read_endpoint].hash
    end
