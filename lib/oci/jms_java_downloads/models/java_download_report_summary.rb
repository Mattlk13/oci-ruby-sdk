...
module OCI
  ...
  class JmsJavaDownloads::Models::JavaDownloadReportSummary
    ...
    SORT_BY_ENUM = [
      ...
      SORT_BY_UNKNOWN_ENUM_VALUE = 'UNKNOWN_ENUM_VALUE'.freeze
    ].freeze

    SORT_ORDER_ENUM = [
      SORT_ORDER_ASC = 'ASC'.freeze,
      SORT_ORDER_DESC = 'DESC'.freeze,
      SORT_ORDER_UNKNOWN_ENUM_VALUE = 'UNKNOWN_ENUM_VALUE'.freeze
    ].freeze
    ...
    # **[Required]** The time the Java download report was created, displayed as an [RFC3339](https://datatracker.ietf.org/doc/html/rfc3339) formatted datetime string.
    # @return [DateTime]
    ...
    def sort_by=(sort_by)
      # rubocop:disable Style/ConditionalAssignment
      if sort_by && !SORT_BY_ENUM.include?(sort_by)
        OCI.logger.debug("Unknown value for 'sort_by' [" + sort_by + "]. Mapping to 'SORT_BY_UNKNOWN_ENUM_VALUE'") if OCI.logger
        @sort_by = SORT_BY_UNKNOWN_ENUM_VALUE
      else
        @sort_by = sort_by
      end
      # rubocop:enable Style/ConditionalAssignment
    ...
    def sort_order=(sort_order)
      # rubocop:disable Style/ConditionalAssignment
      if sort_order && !SORT_ORDER_ENUM.include?(sort_order)
        OCI.logger.debug("Unknown value for 'sort_order' [" + sort_order + "]. Mapping to 'SORT_ORDER_UNKNOWN_ENUM_VALUE'") if OCI.logger
        @sort_order = SORT_ORDER_UNKNOWN_ENUM_VALUE
      else
    ...
    def hash
      [id, display_name, format, file_size_in_bytes, checksum_type, checksum_value, compartment_id, created_by, time_created, time_start, time_end, sort_by, sort_order, lifecycle_state, freeform_tags, defined_tags, system_tags].hash
    end
