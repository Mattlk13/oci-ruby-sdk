...
module OCI
  ...
  class Psql::PostgresqlClient
    ...
    # @note Click [here](https://docs.cloud.oracle.com/en-us/iaas/tools/ruby-sdk-examples/latest/psql/backup_copy.rb.html) to see an example of how to use backup_copy API.
    def backup_copy(backup_id, backup_copy_details, opts = {})
      logger.debug 'Calling operation PostgresqlClient#backup_copy.' if logger

      raise "Missing the required parameter 'backup_id' when calling backup_copy." if backup_id.nil?
      raise "Missing the required parameter 'backup_copy_details' when calling backup_copy." if backup_copy_details.nil?
      raise "Parameter value for 'backup_id' must not be blank" if OCI::Internal::Util.blank_string?(backup_id)

      path = '/backups/{backupId}/actions/copy'.sub('{backupId}', backup_id.to_s)
      ...
      post_body = @api_client.object_to_http_body(backup_copy_details)

      # rubocop:disable Metrics/BlockLength
      OCI::Retry.make_retrying_call(applicable_retry_config(opts), call_name: 'PostgresqlClient#backup_copy') do
        @api_client.call_api(
          :POST,
          path,
          endpoint,
    ...
    # @option opts [String] :compartment_id The ID of the compartment in which to list resources.
    # @option opts [String] :lifecycle_state A filter to return only resources if their `lifecycleState` matches the given `lifecycleState`.
    # @option opts [String] :config_type A filter to return only resources if their `configType` matches the given `configType`.
    # @option opts [String] :display_name A filter to return only resources that match the entire display name given.
    # @option opts [String] :db_version Version of the PostgreSQL database, such as 14.9.
    ...
    def list_configurations(opts = {})
      ...
      if opts[:config_type] && !OCI::Psql::Models::Configuration::CONFIG_TYPE_ENUM.include?(opts[:config_type])
        raise 'Invalid value for "config_type", must be one of the values in OCI::Psql::Models::Configuration::CONFIG_TYPE_ENUM.'
      end
