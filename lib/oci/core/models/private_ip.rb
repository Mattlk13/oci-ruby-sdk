...
module OCI
  ...
  class Core::Models::PrivateIp
    IP_STATE_ENUM = [
      IP_STATE_ASSIGNED = 'ASSIGNED'.freeze,
      IP_STATE_AVAILABLE = 'AVAILABLE'.freeze,
      IP_STATE_UNKNOWN_ENUM_VALUE = 'UNKNOWN_ENUM_VALUE'.freeze
    ].freeze
    ...
    # @option attributes [String] :ip_state The value to assign to the {#ip_state} property
    ...
    def initialize(attributes = {})
      ...
      self.ip_state = attributes[:'ipState'] if attributes[:'ipState']

      raise 'You cannot provide both :ipState and :ip_state' if attributes.key?(:'ipState') && attributes.key?(:'ip_state')

      self.ip_state = attributes[:'ip_state'] if attributes[:'ip_state']
    ...
    # @param [Object] ip_state Object to be assigned
    def ip_state=(ip_state)
      # rubocop:disable Style/ConditionalAssignment
      if ip_state && !IP_STATE_ENUM.include?(ip_state)
        OCI.logger.debug("Unknown value for 'ip_state' [" + ip_state + "]. Mapping to 'IP_STATE_UNKNOWN_ENUM_VALUE'") if OCI.logger
        @ip_state = IP_STATE_UNKNOWN_ENUM_VALUE
      else
        @ip_state = ip_state
      end
    ...
    def hash
      [availability_domain, compartment_id, defined_tags, display_name, freeform_tags, hostname_label, id, ip_address, is_primary, vlan_id, subnet_id, time_created, vnic_id, ip_state, lifetime, route_table_id].hash
    end
