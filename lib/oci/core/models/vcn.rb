...
module OCI
  ...
  class Core::Models::Vcn
    ...
    # @option attributes [BOOLEAN] :is_zpr_only The value to assign to the {#is_zpr_only} property
    def initialize(attributes = {})
      ...
      self.is_zpr_only = attributes[:'isZprOnly'] unless attributes[:'isZprOnly'].nil?
      self.is_zpr_only = false if is_zpr_only.nil? && !attributes.key?(:'isZprOnly') # rubocop:disable Style/StringLiterals

      raise 'You cannot provide both :isZprOnly and :is_zpr_only' if attributes.key?(:'isZprOnly') && attributes.key?(:'is_zpr_only')

      self.is_zpr_only = attributes[:'is_zpr_only'] unless attributes[:'is_zpr_only'].nil?
      self.is_zpr_only = false if is_zpr_only.nil? && !attributes.key?(:'isZprOnly') && !attributes.key?(:'is_zpr_only') # rubocop:disable Style/StringLiterals
    end
    ...
    def hash
      [byoipv6_cidr_blocks, ipv6_private_cidr_blocks, cidr_block, cidr_blocks, compartment_id, default_dhcp_options_id, default_route_table_id, default_security_list_id, defined_tags, display_name, dns_label, freeform_tags, security_attributes, id, ipv6_cidr_blocks, lifecycle_state, time_created, vcn_domain_name, is_zpr_only].hash
    end
