...
module OCI
  ...
  class AiSpeech::Models::RealtimeSessionToken
    # **[Required]** The session token (JWT) to be consumed by the websocket server. The token contains the session/tenant ID, as well as the expiry time.
    # @return [String]
    attr_accessor :token

    # **[Required]** The session ID this token corresponds to. Provided for convenience, the session ID is already present in the JWT token.
    # @return [String]
    attr_accessor :session_id

    # **[Required]** Compartment ID that was used to create the token.
    # @return [String]
    ...
    def hash
      [token, session_id, compartment_id, freeform_tags, defined_tags].hash
    end
