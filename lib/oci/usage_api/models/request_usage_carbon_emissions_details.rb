...
module OCI
  ...
  class UsageApi::Models::RequestUsageCarbonEmissionsDetails
    EMISSION_CALCULATION_METHOD_ENUM = [
      EMISSION_CALCULATION_METHOD_SPEND_BASED = 'SPEND_BASED'.freeze,
      EMISSION_CALCULATION_METHOD_POWER_BASED = 'POWER_BASED'.freeze,
      EMISSION_CALCULATION_METHOD_UNKNOWN_ENUM_VALUE = 'UNKNOWN_ENUM_VALUE'.freeze
    ].freeze

    EMISSION_TYPE_ENUM = [
      EMISSION_TYPE_MARKET_BASED = 'MARKET_BASED'.freeze,
      EMISSION_TYPE_LOCATION_BASED = 'LOCATION_BASED'.freeze,
      EMISSION_TYPE_UNKNOWN_ENUM_VALUE = 'UNKNOWN_ENUM_VALUE'.freeze
    ].freeze
    ...
    # @param [Object] emission_calculation_method Object to be assigned
    def emission_calculation_method=(emission_calculation_method)
      # rubocop:disable Style/ConditionalAssignment
      if emission_calculation_method && !EMISSION_CALCULATION_METHOD_ENUM.include?(emission_calculation_method)
        OCI.logger.debug("Unknown value for 'emission_calculation_method' [" + emission_calculation_method + "]. Mapping to 'EMISSION_CALCULATION_METHOD_UNKNOWN_ENUM_VALUE'") if OCI.logger
        @emission_calculation_method = EMISSION_CALCULATION_METHOD_UNKNOWN_ENUM_VALUE
      else
        @emission_calculation_method = emission_calculation_method
      end
      # rubocop:enable Style/ConditionalAssignment
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] emission_type Object to be assigned
    def emission_type=(emission_type)
      # rubocop:disable Style/ConditionalAssignment
      if emission_type && !EMISSION_TYPE_ENUM.include?(emission_type)
        OCI.logger.debug("Unknown value for 'emission_type' [" + emission_type + "]. Mapping to 'EMISSION_TYPE_UNKNOWN_ENUM_VALUE'") if OCI.logger
        @emission_type = EMISSION_TYPE_UNKNOWN_ENUM_VALUE
      else
    ...
    def hash
      [tenant_id, time_usage_started, time_usage_ended, emission_calculation_method, emission_type, granularity, is_aggregate_by_time, group_by, group_by_tag, compartment_depth, filter].hash
    end
