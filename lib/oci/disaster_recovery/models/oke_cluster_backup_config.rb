...
module OCI
  ...
  #
  class DisasterRecovery::Models::OkeClusterBackupConfig
    REPLICATE_IMAGES_ENUM = [
      REPLICATE_IMAGES_ENABLE = 'ENABLE'.freeze,
      REPLICATE_IMAGES_DISABLE = 'DISABLE'.freeze,
      REPLICATE_IMAGES_UNKNOWN_ENUM_VALUE = 'UNKNOWN_ENUM_VALUE'.freeze
    ].freeze
    ...
    def initialize(attributes = {})
      ...
      self.backup_schedule = "FREQ=WEEKLY;INTERVAL=1" if backup_schedule.nil? && !attributes.key?(:'backupSchedule') # rubocop:disable Style/StringLiterals
      ...
      self.backup_schedule = "FREQ=WEEKLY;INTERVAL=1" if backup_schedule.nil? && !attributes.key?(:'backupSchedule') && !attributes.key?(:'backup_schedule') # rubocop:disable Style/StringLiterals
    ...
    def replicate_images=(replicate_images)
      ...
      if replicate_images && !REPLICATE_IMAGES_ENUM.include?(replicate_images)
        OCI.logger.debug("Unknown value for 'replicate_images' [" + replicate_images + "]. Mapping to 'REPLICATE_IMAGES_UNKNOWN_ENUM_VALUE'") if OCI.logger
        @replicate_images = REPLICATE_IMAGES_UNKNOWN_ENUM_VALUE
      else
