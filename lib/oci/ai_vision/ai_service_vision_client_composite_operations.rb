...
module OCI
  ...
  class AiVision::AIServiceVisionClientCompositeOperations
    ...
    # Calls {OCI::AiVision::AIServiceVisionClient#create_video_job} and then waits for the {OCI::AiVision::Models::VideoJob} acted upon
    # to enter the given state(s).
    #
    # @param [OCI::AiVision::Models::CreateVideoJobDetails] create_video_job_details Details about the video analysis.
    # @param [Array<String>] wait_for_states An array of states to wait on. These should be valid values for {OCI::AiVision::Models::VideoJob#lifecycle_state}
    # @param [Hash] base_operation_opts Any optional arguments accepted by {OCI::AiVision::AIServiceVisionClient#create_video_job}
    # @param [Hash] waiter_opts Optional arguments for the waiter. Keys should be symbols, and the following keys are supported:
    #   * max_interval_seconds: The maximum interval between queries, in seconds.
    #   * max_wait_seconds The maximum time to wait, in seconds
    #
    # @return [OCI::Response] A {OCI::Response} object with data of type {OCI::AiVision::Models::VideoJob}
    def create_video_job_and_wait_for_state(create_video_job_details, wait_for_states = [], base_operation_opts = {}, waiter_opts = {})
      operation_result = @service_client.create_video_job(create_video_job_details, base_operation_opts)
      ...
      begin
        waiter_result = @service_client.get_video_job(wait_for_resource_id).wait_until(
          eval_proc: ->(response) { response.data.respond_to?(:lifecycle_state) && lowered_wait_for_states.include?(response.data.lifecycle_state.downcase) },
          max_interval_seconds: waiter_opts.key?(:max_interval_seconds) ? waiter_opts[:max_interval_seconds] : 30,
          max_wait_seconds: waiter_opts.key?(:max_wait_seconds) ? waiter_opts[:max_wait_seconds] : 1200
        )
